...
PATH = os.path.join(OUT, "cells_odpair.csv")
def months(a,b):
    ...
    return out
MONTHS = months("2024-01","2025-06")
def done():
    if not os.path.exists(PATH): return set()
    with open(PATH) as f:
        next(f,None); return {l.split(",")[0] for l in f}
def fetch(m):
    dst=os.path.join(CACHE,f"od_{m}.parquet")
...
for m in MONTHS:
    ...
    SELECT '{m}' ym, PULocationID pu,
      ...
      SUM(CASE WHEN payment_type=1 THEN fare_amount END) s_fare,
    ...
    with open(PATH,"a") as f:
        if new: f.write("ym,pu,dozone,n_all,n_card,s_tip,s_ptt,s_fare,s_cbd,s_cs\n")
        for r in rows: f.write(",".join("" if x is None else str(x) for x in r)+"\n")
    print(f"{m}: +{len(rows)}",flush=True)
    try: os.remove(u)
    except OSError: pass
print("ODDONE",flush=True)
